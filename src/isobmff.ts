...
class Parser {
  ...
  async parse(): Promise<Essentials | undefined> {
    ...
    const type = await this.reader.readAsciiString(4);
    if (type !== "ftyp") {
      return;
    }

    const brand = await this.reader.readAsciiString(4);
  ...
  private async parseIsom(): Promise<Essentials | undefined> {
    ...
    for (let keyIndex = 0; keyIndex < keysCount; ++keyIndex) {
      ...
      const value = await this.reader.readAsciiString(size);
      if (
    ...
    const ilstReader = this.reader.getSubreader(box.size);
    this.reader.incrementOffset(box.size);
    while (true) {
